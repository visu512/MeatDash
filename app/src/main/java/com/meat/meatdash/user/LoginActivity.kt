...
class LoginActivity : AppCompatActivity() {
    ...
    override fun onStart() {
        super.onStart()
        auth.currentUser?.let { redirectToPhoneActivity() }
    }
    ...
    private fun setupClickListeners() {
        ...
        binding.privacyPolicyText.setOnClickListener {
            startActivity(Intent(Intent.ACTION_VIEW, Uri.parse("https://privicy-policies.netlify.app/")))
        }
        ...
        auth.signInWithEmailAndPassword(email, password)
            .addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    ...
                    showToast("Login failed: ${task.exception?.message}", DesignerToast.STYLE_DARK)
                }
            }
    }
                ...
                auth.signInWithCredential(credential)
                    .addOnCompleteListener(this) { task ->
                        if (task.isSuccessful) {
                            ...
                            showToast("Google sign in successful!", DesignerToast.STYLE_DARK, isSuccess = true)
                            redirectToPhoneActivity()
                        } else {
                            Log.w(TAG, "signInWithCredential:failure", task.exception)
                        }
    ...
    private fun showToast(message: String, style: String, isSuccess: Boolean = false) {
        if (isSuccess) {
            DesignerToast.Success(this, "Success", message, Gravity.TOP, Toast.LENGTH_SHORT, style)
        } else {
            DesignerToast.Error(this, "Error", message, Gravity.TOP, Toast.LENGTH_SHORT, style)
        }
    }
}
